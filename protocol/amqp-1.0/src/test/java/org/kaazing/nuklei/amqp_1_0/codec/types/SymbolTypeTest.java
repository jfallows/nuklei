...
public class SymbolTypeTest
{
    ...
    public void shouldEncode4(int offset)
    {
        ...
        assertEquals(offset + 261, symbolType.limit());
    }

    @Theory
    public void shouldDecode1(int offset)
    {
        buffer.putByte(offset, (byte) 0xa3);
    ...
    {
        buffer.putByte(offset, (byte) 0xb3);
        ...
        char[] chars = new char[256];
        Arrays.fill(chars, 'a');
        assertEquals(new String(chars), symbolType.get(READ_UTF_8).toString());
        assertEquals(offset + 261, symbolType.limit());
    }
